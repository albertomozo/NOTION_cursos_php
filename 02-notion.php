...
//print_r ($respuesta);
$listacurso = '<ul>';
foreach ($respuesta['results'] as $key => $valor){
    // un ciclo por cada curso
    /* $listacurso = '<li>' .
    $valor['propierties']['Name'].title[0]['plain_text'] . '</li>'; */
    foreach ($valor['properties'] as $key1 => $valor1){ 
        // propeidades del curso
       // print_r ($valor1);
        //echo '<hr>';
        if ($valor1['id']=='title'){
            //print_r ($valor1);
            //echo '<hr>';
            foreach ($valor1['title'] as $key2 => $valor2){
                echo $valor2['plain_text'];
                echo '<hr>'; 
            }
